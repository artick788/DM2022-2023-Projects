...
protected:
    ...
    void getsubs(unsigned sl, unsigned min, unsigned *iset, set<sItem> *items, unsigned start, unsigned depth,
                 vector<int> &V, vector<int> &P);

    void propagate(vector<int> &V, int vb, int v, vector<int> &P, int pb, int p, int m, int n);

    void getBounds(vector<int> &V, int &l, int &u, unsigned sl, unsigned min, unsigned start, unsigned depth,
                   unsigned &index);
    ...
    int processTransaction1(unsigned level, Transaction *t, set<Item> *items, unsigned spos, unsigned depth,
                            unsigned suppfirstitem = 0);
