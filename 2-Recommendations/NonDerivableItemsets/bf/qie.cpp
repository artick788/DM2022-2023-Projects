...
void NDI::getsubs(unsigned sl, unsigned min, unsigned *iset, set<sItem> *items, unsigned start, unsigned depth,
...
void NDI::propagate(vector<int> &V, int vb, int v, vector<int> &P, int pb, int p, int m, int n) {
    ...
    if (m == 0) {
        ...
        if (v > 2) {
            ...
            propagate(V, vb + v_split, v - v_split, P, pb + p_split + (m ? 0 : 1), p - p_split - (m ? 0 : 1),
                      (m ? m - 1 : 0), n - 1);
        }
    }
}
...
void NDI::getBounds(vector<int> &V, int &l, int &u, unsigned sl, unsigned min, unsigned start, unsigned depth,
                    unsigned &index) {
    unsigned stop = (depth < min ? sl - (min - depth) : sl);
    if (depth < sl)
        for (unsigned i = stop - 1; i >= start && i < stop; i--) {
