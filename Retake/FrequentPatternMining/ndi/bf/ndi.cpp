...
int NDI::processTransaction(unsigned level, Transaction *t, set<Item> *items, unsigned spos, unsigned depth) {
    if ((level == 2) || (items->size() > t->length - level + depth - spos))
        return processTransaction1(level, t, items, spos, depth);
...
int NDI::processTransaction1(unsigned level, Transaction *t, set<Item> *items, unsigned spos, unsigned depth,
                             unsigned suppfirstitem) {
    if (items == 0) return 0;
    int used = 0;
    ...
    while (spos < max) {
        ...
        if (runner != items->end()) {
            if (depth == level) {
                ...
                if (depth == 1 && level == 2) {
                    runner->makeChildren();
                    used += processTransaction1(level, t, runner->getChildren(), spos + 1, depth + 1,
                                                runner->getSupport());
    ...
    for (set<Item>::iterator runner = items->begin(); runner != items->end();) {
        ...
        if (depth == level) {
            ...
            if (runner->getSupport() < minsup) {
                ...
                if (((int) runner->getSupport() == runner->getLB()) ||
                    ((int) runner->getSupport() == runner->getUB())) {
                    set<Item>::iterator tmp = runner++;
                    items->erase(tmp);
                } else {
